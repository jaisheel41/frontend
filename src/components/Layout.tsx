...
const Layout: React.FC = () => {
  ...
  // Scroll event listener to update activeSection
  useEffect(() => {
    const handleScroll = () => {
      for (const section of sections) {
        const element = document.getElementById(section.name);
  ...
  // Scroll to a section when a link is clicked
  const scrollToSection = (id: string) => {
    const element = document.getElementById(id);
    if (element) {
      element.scrollIntoView({ behavior: "smooth" });
        ...
        <nav className="hidden md:flex flex-1 justify-center space-x-8">
          {sections.map(({ name }) => (
            <a
              ...
              onClick={() => scrollToSection(name)} // Update activeSection on click
            >
              {name}
            </a>
          ))}
        ...
        <div className="md:hidden fixed top-14 left-0 w-full bg-[#1e1e2e] shadow-lg py-4 flex flex-col items-center space-y-4 z-50">
          {sections.map(({ name }) => (
            <a
              ...
              onClick={() => {
                setMenuOpen(false);
                scrollToSection(name); // Update activeSection on click
              }}
            >
              {name}
            </a>
